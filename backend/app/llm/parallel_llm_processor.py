    ...
    async def call_api(
        ...
        try:
            ...
            if response.status == 429:
                ...
                logger.error(f"Request {self.task_id} API error {response.status}: {response_json}")
                error = response_json
        ...
        if error:
            if self.attempts_left > 0:
                backoff_seconds = self.backoff_base_seconds * (2 ** (3 - self.attempts_left)) + random.uniform(0, 1)
                logger.info(f"Retrying request {self.task_id} after {backoff_seconds:.2f}s backoff")
...
def num_tokens_consumed_from_request(request_json: dict, token_encoding_name: str) -> int:
        ...
        while True:
                ...
                if not queue_of_requests_to_retry.empty():
                    ...
                    try:
                        ...
                        next_request = APIRequest(
                            ...
                            token_consumption=num_tokens_consumed_from_request(request_json, token_encoding_name),
            ...
            if next_request:
                if (available_request_capacity >= 1 and
                        available_token_capacity >= next_request.token_consumption and
                        len(in_flight_tasks) < max_concurrent_requests):
            ...
            if status_tracker.num_tasks_in_progress == 0 and not requests_not_finished and queue_of_requests_to_retry.empty():
                if in_flight_tasks:
            ...
            if status_tracker.last_rate_limit_error_time:
                ...
                if time_since_last_rl < base_cooldown_seconds:
                    ...
                    logger.info(f"Cooling down for {cooldown_remaining:.1f}s due to 429s...")
...
class RequestProcessor:
    ...
    async def process_requests(self, requests: List[dict], max_attempts: int = 2) -> List[dict]:
